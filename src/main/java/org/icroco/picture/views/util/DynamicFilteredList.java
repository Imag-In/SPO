...
import org.icroco.picture.util.CustomSortHelper;
 ...
 */
@Slf4j
public final class DynamicFilteredList<E> extends TransformationList<E, E> {

    private int[] filtered;
    private int   size;

    private CustomSortHelper helper;
     ...
     */
    public DynamicFilteredList(@NamedArg("source") ObservableList<E> source, @NamedArg("predicate") Predicate<? super E> predicate) {
        super(source);
     ...
     * @param source the source list
     */
    public DynamicFilteredList(@NamedArg("source") ObservableList<E> source) {
        this(source, null);
    }
        ...
        if (predicate == null) {
            predicate = new ObjectPropertyBase<>() {
                ...
                public Object getBean() {
                    return DynamicFilteredList.this;
                }
    ...
    public void setPredicate(Predicate<? super E> predicate) {
        ...
        if (predicate instanceof MultiplePredicates<? super E> multiplePredicates) {
            multiplePredicates.getPredicates().addListener((ListChangeListener.Change<?> _) -> {
                log.info("Refilter");
                refilter();
            });
//            multiplePredicates.getPredicates().addListener((Observable _) -> refilter());
    ...
    private CustomSortHelper getSortHelper() {
        if (helper == null) {
            helper = new CustomSortHelper();
        }
        return helper;
    }
    ...
    private void refilter() {
        ...
        if (hasListeners()) {
            fireChange(new NonIterableChange.GenericAddRemoveChange<E>(0, size, removed, this));
        }
    }
