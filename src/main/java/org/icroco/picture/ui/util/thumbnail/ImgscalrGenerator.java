...
//                byte[]        jpgs   = ImageUtils.toByteArray(t, "jpg");
    ...
    public void generate(Path source, Path target, Dimension dim) {
        try {
            var img         = ImageIO.read(source.toFile()); // load image
            var orientation = metadataExtractor.orientation(source).orElse(1);
            deleteFile(target);
            img = resize(img, dim);
            img = adaptOrientation(img, orientation);
     ...
     * {@link #DEBUG} is <code>true</code>; otherwise the VM will be
     ...
     * {@link #THRESHOLD_BALANCED_SPEED} thresholds when selecting which
    ...
    protected static BufferedImage scaleImage(BufferedImage src, int targetWidth, int targetHeight, Object interpolationHintValue) {
        // Set up the rendering resources to match the source image's
