...
import org.icroco.picture.ui.util.FileResource;
...
public final class SamplerTheme implements Theme {
    ...
    public SamplerTheme(Theme theme, Env env) {
    ...
    public Set<String> getAllStylesheets() {
        return env.isDev() ? merge(getResource().toURI().toString(), ThemeManager.APP_STYLESHEETS) : Set.of(ThemeManager.APP_STYLESHEETS);
    }
    ...
    public FileResource getResource() {
        ...
        try {
            FileResource testTheme = FileResource.createInternal(
                    Resources.resolve("theme-test/" + filename), ImagInApp.class
            );
            if (!testTheme.exists()) {
            ...
            var failedPath = Resources.resolve("theme-test/" + filename);
            System.err.println(
